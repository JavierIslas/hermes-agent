...
def _pre_tool_block(agent, ref: _ToolCallRef):
    """Run ``pre_tool_call`` plugin hooks; returns ``(block_message, final_args, halt_loop)``
    with any hook-modified args applied. Hook failures never block.

    ``halt_loop`` — Fix D5 (arnes-gates): a plugin ``block`` directive may
    escalate into a real ToolGuardrailDecision(halt); the caller breaks the
    conversation loop instead of burning iterations on a tripped breaker."""
    try:
        from hermes_cli.plugins import _dispatch_pre_tool_call_hooks

        block_msg, modified_args, halt_loop = _dispatch_pre_tool_call_hooks(
            ref.name,
            ref.args,
            **tool_hook_ids(agent, ref.task_id, ref.call_id),
            middleware_trace=list(ref.trace),
        )
        return block_msg, (ref.args if modified_args is None else modified_args), halt_loop
    except Exception:
        return None, ref.args, False
...
def _dispatch_authorized_once(
    ...
    *,
    ...
    if block_message is None:
        ...
        resolved = resolve() if authorization_gate is None else authorization_gate.run(resolve)
        block_message, ref.args, halt_loop = resolved
        state.args = ref.args
    ...
    if block_message is not None or guardrail_decision is not None:
        ...
        state.blocked = True
        # Fix D5 (arnes-gates): a plugin block carrying halt_loop escalates
        # to a real ToolGuardrailDecision(halt) so the conversation loop
        # breaks (guardrail_halt) instead of burning iterations against a
        # tripped circuit breaker.
        if block_message is not None and halt_loop:
            from agent.tool_guardrails import ToolGuardrailDecision

            controller = getattr(agent, "_tool_guardrails", None)
            try:
                failures = (
                    controller.same_tool_failure_count(ref.name)
