...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if getattr(agent, "worker_mode", False):
        # Worker mode: the identity slot belongs to WORKER.md (installed by
        # the arnes-gates plugin, editable by the operator). Falls back to
        # DEFAULT_AGENT_IDENTITY when the file is absent — core stays
        # healthy without the plugin.
