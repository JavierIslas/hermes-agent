...
def _identity_parts(agent: Any, ctx_len: Optional[int]) -> Tuple[List[str], bool]:
    """Identity slot #1: WORKER.md in worker mode, else SOUL.md (cron keeps the
    persona while skipping cwd instructions, scoped to the agent's OWN home),
    else the default identity. Returns ``(parts, soul_loaded)``."""
    if getattr(agent, "worker_mode", False):
        ...
        _worker_identity = _pb.load_worker_md(ctx_len)
        if _worker_identity:
            return ([_worker_identity], True)
