...
def load_worker_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    if not worker_path.exists():
        return None
    try:
        content = (_read_text_with_timeout(worker_path) or "").strip()
        if not content:
            return None
        return _truncate_content(
            _scan_context_content(content, "WORKER.md"), "WORKER.md",
            context_length=context_length, read_path=str(worker_path),
        )
    except Exception as e:
